function [xt,xtd] = matrixTrigPolynomial(Xa,Xb,t_k,par)
%*************************************************************************
...
%************************************************************************
    M = par.M; % degree of polynomial    
    w = par.w; % frequency    
    
    xt = Xa(:,:,1);             % R_{a,0} [Saetre, p.33]
    xtd = zeros(size(Xa,1));    % derivative of R
    % build trigonometric polynomial sum_{1}^{M} ( cos[kw*t_{l}]*R_{a,k} 
    % + sin[kw*t_{l}]*R_{b,k})
    % and its derivative
    for k = 1:M
